...
namespace nc
{
	struct Vector2
	{
		...
		Vector2 operator - ()const { return Vector2{ x * -1, y * -1 }; }

		bool operator == (const Vector2& v) const { return x == v.x && y == v.y; }
		bool operator != (const Vector2& v) const { return x != v.x && y != v.y; }

		bool operator < (float s) const { return x + y < s; }
		bool operator > (float s) const { return x + y > s; }
		...
		static Vector2 CircleClamp(const Vector2& v, float radius);
		void CircleClamp(float radius);
	...
	inline Vector2 Vector2::Normalized() const { return *this / Length(); }
